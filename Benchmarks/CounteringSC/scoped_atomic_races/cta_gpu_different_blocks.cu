...
__global__ void cta_gpu_different_blocks(unsigned int *value) {
...
int main() {
    ...
    cta_gpu_different_blocks<<<2, 1>>>(device_value);
    checkCuda(cudaGetLastError(), "launch cta_gpu_different_blocks");
    checkCuda(cudaDeviceSynchronize(), "sync cta_gpu_different_blocks");
    ...
    std::printf(
        "CTA-vs-GPU scope atomics: final=%u hardware_expected_if_globally_serialized=%u\n",
        host_value,
        hardware_expected);
